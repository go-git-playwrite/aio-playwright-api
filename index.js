...
function getCoverageRepresentativeRejectReason_(candidate) {
  ...
  if (/\/(?:facebook|instagram|twitter|x\.com|youtube|line|linkedin|tiktok)(?:\/|$|-|_)/i.test(path)) return 'social_url';
...
function preserveSelectedCoverageRepresentatives_(coverageSignalsV1, selectedCandidates, opts = {}) {
  ...
  const preserved = selectedRepresentativePages.map(candidatePage => {
    ...
    return Object.assign({}, candidatePage, observed, {
      ...
      bodyTextLength: Number(observed.bodyTextLength || 0) || 0,
      jsonLdCount: Number(observed.jsonLdCount || 0) || (Array.isArray(observed.jsonLdTypes) ? observed.jsonLdTypes.length : 0),
      hasMain: observed.hasMain === true,
      hasMainLandmark: observed.hasMainLandmark === true,
      hasNavLike: observed.hasNavLike === true || observed.navLikeFound === true,
      hasFooterLike: observed.hasFooterLike === true || observed.footerLikeFound === true,
...
function buildRepresentativeObservationQuality_(page) {
  ...
  const navigationCommitted = page && Object.prototype.hasOwnProperty.call(page, 'navigationCommitted')
    ...
    : null;
  const timedOut = /timeout/i.test(String(page && page.error || ''));
  const partial = page && (page.returnedPartial === true || page.extractTimedOut === true || page.pageBudgetExceeded === true);
  const contentTextLength = Number(page && page.bodyTextLength || 0) || normalizeSubpageJsonLdText(page && page.sampledText).length;
  const titlePresent = !!normalizeSubpageJsonLdText(page && page.title);
  const h1Count = Number(page && page.h1Count || 0) || (normalizeSubpageJsonLdText(page && page.h1) ? 1 : 0);
  const jsonLdCount = Number(page && page.jsonLdCount || 0) || (Array.isArray(page && page.jsonLdTypes) ? page.jsonLdTypes.length : 0);
  const mainLikeFound = page && (page.hasMain === true || page.hasMainLandmark === true);
  const navLikeFound = page && (page.hasNavLike === true || page.navLikeFound === true);
  const footerLikeFound = page && (page.hasFooterLike === true || page.footerLikeFound === true);
  const internalLinkCount = Number(page && page.internalLinkCount || 0) || 0;
  const sameOriginLinkCount = Number(page && page.sameOriginLinkCount || 0) || internalLinkCount;
  const signals = {
    hasTitle: titlePresent,
    hasH1: h1Count > 0,
    hasJsonLd: jsonLdCount > 0,
    hasMainLike: mainLikeFound === true,
    hasNavLike: navLikeFound === true,
    hasFooterLike: footerLikeFound === true,
    hasEnoughText: contentTextLength >= 300
  };
  const reasons = [];
  let level = 'failed';
  if (!reached) {
    reasons.push('not_reached');
  } else if (signals.hasEnoughText && (signals.hasTitle || signals.hasH1) && (signals.hasMainLike || signals.hasJsonLd || signals.hasNavLike || signals.hasFooterLike)) {
    level = 'good';
    reasons.push('sufficient_light_signals');
  } else if ((signals.hasTitle || signals.hasH1 || signals.hasJsonLd) || contentTextLength >= 100) {
    level = 'partial';
    reasons.push('partial_light_signals');
  } else {
    level = 'weak';
    reasons.push('reached_but_weak_light_signals');
  }
  ...
  return {
    attempted: page && page.observationAttempted === true,
    ...
    h1Count,
    jsonLdCount,
    mainLikeFound: mainLikeFound === true,
    navLikeFound: navLikeFound === true,
    footerLikeFound: footerLikeFound === true,
    internalLinkCount,
    sameOriginLinkCount,
    signals,
    level,
...
function attachRepresentativeObservationQuality_(coverageSignals) {
  if (!coverageSignals || typeof coverageSignals !== 'object') return coverageSignals;
  ...
  const summary = coverageSignals.representativePages.reduce((acc, page) => {
    ...
    acc.total += 1;
    if (q.attempted === true) acc.attempted += 1;
    if (q.reached === true) acc.reached += 1;
    if (q.level && Object.prototype.hasOwnProperty.call(acc, q.level)) acc[q.level] += 1;
    if (page && page.candidateOnly === true) acc.candidateOnly += 1;
    if (page && page.candidateOnly !== true) acc.observed += 1;
    return acc;
...
function logRepresentativeObservationQualityPhase12_(payload = {}) {
  try {
    ...
    console.log('[DEBUG][REPRESENTATIVE_OBSERVATION_QUALITY_PHASE12]', JSON.stringify({
      origin: String(payload.origin || '').slice(0, 180),
      selectedCount: typeof payload.selectedCount === 'number' ? payload.selectedCount : pages.length,
      observedCount: typeof coverageSignals.observedCount === 'number' ? coverageSignals.observedCount : (typeof coverageSignals.observedSubpageCount === 'number' ? coverageSignals.observedSubpageCount : null),
      qualitySummary: coverageSignals.representativeObservationQuality || {},
      pages: pages.slice(0, 8).map(page => {
        ...
        return {
          ...
          metrics: {
            ...
            internalLinkCount: Number(q.internalLinkCount || 0),
            sameOriginLinkCount: Number(q.sameOriginLinkCount || 0)
          }
        };
      })
    }));
...
function buildCoverageSignalsV1FromSubpageObservation_(payload) {
  ...
  attachRepresentativeObservationQuality_(out);
  return out;
}
...
function buildGeoSignalsCoverageSignals_(coverageSignalsV1) {
  ...
  const representativePagesInput = Array.isArray(coverageSignalsV1.representativePages)
    ? coverageSignalsV1.representativePages
    : [];
  const candidatePageTypesInput = coverageSignalsV1.candidatePageTypes || buildCoverageCandidatePageTypes_([]);
  const observedSubpageCount = Number(coverageSignalsV1.observedSubpageCount || 0);
  const hasCandidatePageType = Object.keys(candidatePageTypesInput || {}).some(key => candidatePageTypesInput[key] === true);
  const out = {
    version: 'coverageSignalsV1',
    source: 'discover-and-observe-subpages-light',
    checked: true,
    observedSubpageCount,
    observedCount: observedSubpageCount,
    hasSubpageSignals: observedSubpageCount > 0 || representativePagesInput.length > 0 || hasCandidatePageType,
    ...
    representativePages: representativePagesInput.slice(0, 5).map(page => ({
      ...
      hasNavLike: page && page.hasNavLike === true,
...
function logSubpageCoverageSignalsFinalAudit_(payload = {}) {
  try {
    ...
    console.log('[DEBUG][SUBPAGE_COVERAGE_SIGNALS_FINAL_AUDIT]', JSON.stringify({
      ...
      mode: payload.mode || 'signalsMode=light',
      origin: String(payload.origin || ''),
      hasGeoSignalsV1: payload.hasGeoSignalsV1 === true,
      hasCoverageSignals: !!(payload.coverageSignals && typeof payload.coverageSignals === 'object'),
      hasSubpageSignals: !!(payload.subpageSignals && typeof payload.subpageSignals === 'object'),
...
function logSubpageObservationPhase11_(payload = {}) {
  try {
    const coverageSignals = payload.coverageSignals && typeof payload.coverageSignals === 'object'
      ? payload.coverageSignals
      : {};
    const subpageSignals = payload.subpageSignals && typeof payload.subpageSignals === 'object'
      ? payload.subpageSignals
      : null;
    const state = payload.state && typeof payload.state === 'object' ? payload.state : {};
    const representativePages = Array.isArray(coverageSignals.representativePages)
      ? coverageSignals.representativePages
      : [];
    console.log('[DEBUG][SUBPAGE_OBSERVATION_PHASE11]', JSON.stringify({
      debugRunId: state.debugRunId || null,
      origin: String(payload.origin || ''),
      url: String(payload.url || ''),
      responseMode: payload.responseMode || null,
      hasCoverageSignals: !!(payload.coverageSignals && typeof payload.coverageSignals === 'object'),
      hasSubpageSignals: !!subpageSignals,
      representativePagesCount: representativePages.length,
      representativePages: representativePages.slice(0, 10).map(page => ({
        path: String(page && page.path || '').slice(0, 160),
        pageType: String(page && page.pageType || '').slice(0, 80),
      ...
      observedCount: typeof coverageSignals.observedCount === 'number'
        ? coverageSignals.observedCount
        : (typeof coverageSignals.observedSubpageCount === 'number'
          ? coverageSignals.observedSubpageCount
          : (subpageSignals && typeof subpageSignals.observedCount === 'number' ? subpageSignals.observedCount : null)),
      candidatePageTypes: coverageSignals.candidatePageTypes || {},
      observationStarted: state.observationStarted === true,
      observationCompleted: state.observationCompleted === true,
      fallbackReason: state.fallbackReason || null,
      memoryGuardTriggered: state.memoryGuardTriggered === true,
      timeoutTriggered: state.timeoutTriggered === true,
      errorName: state.errorName || null,
...
async function attachCoverageSignalsToGeoSignalsLight_(geoSignalsV1, topUrl, opts = {}) {
  ...
  try {
    ...
    if (!geoSignalsV1 || typeof geoSignalsV1 !== 'object') {
      ...
      setPhase11State({ fallbackReason: 'missing_geo_signals' });
    ...
    if (!normalized.ok) {
      ...
      setPhase11State({ fallbackReason: logPayload.reason });
    ...
    if (shouldSkipSubpageForMemoryGuard) {
      ...
      if (geoSignalsV1 && geoSignalsV1.coverageSignals && typeof geoSignalsV1.coverageSignals === 'object') {
        ...
        attachRepresentativeObservationQuality_(geoSignalsV1.coverageSignals);
      }
      ...
      logSubpageCoverageSignalsFinalAudit_({
        ...
        coverageSignals: geoSignalsV1 && geoSignalsV1.coverageSignals,
        subpageSignals: geoSignalsV1 && geoSignalsV1.subpageSignals,
        responseMode: 'pre_response_memory_guard'
      });
      logRepresentativeObservationQualityPhase12_({
        origin: normalized.origin,
        selectedCount: candidateOnlyRepresentativePages.length,
        coverageSignals: geoSignalsV1 && geoSignalsV1.coverageSignals
      });
      return emptyCoverageSignals;
    }
    ...
    if (observations.some(page => /timeout/i.test(String(page && page.error || '')))) {
      setPhase11State({
    ...
    if (!coverageSignals) {
      ...
      setPhase11State({ fallbackReason: 'no_observed_subpages' });
    ...
    logRepresentativeObservationQualityPhase12_({
      origin: normalized.origin,
      selectedCount: selectedCandidates.length,
      coverageSignals: geoSignalsV1.coverageSignals
    });
    logRepresentativeSelectionPhase11_({
      debugRunId: phase11DebugRunId,
      origin: normalized.origin,
    ...
    logSubpageCoverageSignalsFinalAudit_({
      ...
      coverageSignals,
      subpageSignals: geoSignalsV1 && geoSignalsV1.subpageSignals,
      responseMode: 'pre_response_attach'
    });
    ...
    logSubpageCoverageSignalsFinalAudit_({
      ...
      origin: normalized && normalized.origin || '',
      hasGeoSignalsV1: !!geoSignalsV1,
      coverageSignals: emptyCoverageSignals,
      subpageSignals: geoSignalsV1 && geoSignalsV1.subpageSignals,
      responseMode: 'pre_response_fallback'
    });
...
function buildBalancedShortResponsePayload(fullPayload) {
  ...
  const shortGeoSignalsV1 = {
    ...
    coverageSignals: g.coverageSignals || null,
    subpageSignals: g.subpageSignals || null
  };
    ...
    if (signalsFirstLight || signalsFirstBalanced) {
      ...
      if (balancedShortResponse) {
        ...
        logSubpageCoverageSignalsFinalAudit_({
          ...
          hasGeoSignalsV1: !!(shortPayload && shortPayload.geoSignalsV1),
        ...
        logSubpageObservationPhase11_({
          state: subpageObservationPhase11State,
          origin: (() => { try { return new URL(String(shortPayload && (shortPayload.finalUrl || shortPayload.url) || '')).origin; } catch (_) { return ''; } })(),
          url: shortPayload && shortPayload.url,
          responseMode: shortPayload && shortPayload.responseMode,
          coverageSignals: shortPayload && shortPayload.geoSignalsV1 && shortPayload.geoSignalsV1.coverageSignals,
          subpageSignals: shortPayload && shortPayload.geoSignalsV1 && shortPayload.geoSignalsV1.subpageSignals
        });
      ...
      logSubpageCoverageSignalsFinalAudit_({
        ...
        hasGeoSignalsV1: !!(signalsResponsePayload && signalsResponsePayload.geoSignalsV1),
      ...
      logSubpageObservationPhase11_({
        state: subpageObservationPhase11State,
        origin: (() => { try { return new URL(String(signalsResponsePayload && (signalsResponsePayload.finalUrl || signalsResponsePayload.url) || '')).origin; } catch (_) { return ''; } })(),
        url: signalsResponsePayload && signalsResponsePayload.url,
        responseMode: signalsResponsePayload && signalsResponsePayload.responseMode || 'default',
        coverageSignals: signalsResponsePayload && signalsResponsePayload.geoSignalsV1 && signalsResponsePayload.geoSignalsV1.coverageSignals,
        subpageSignals: signalsResponsePayload && signalsResponsePayload.geoSignalsV1 && signalsResponsePayload.geoSignalsV1.subpageSignals
      });
