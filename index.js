...
async function fetchSubpagePlaywrightScopedLight(url, opts = {}) {
  ...
  try {
    ...
    const readScopedReadiness = async () => page.evaluate(() => {
      ...
      const roleMainCount = document.querySelectorAll('[role="main"]').length;
      let reason = '';
      if (bodyChildCount > 0) reason = 'body_child_observed';
      else if (bodyText.length > 100) reason = 'body_text_observed';
      else if (h1Count + h2Count > 0) reason = 'heading_observed';
      else if (sameOriginLinkCount > 0) reason = 'same_origin_link_observed';
      else if (mainCount + articleCount + roleMainCount > 0) reason = 'main_like_observed';
      return {
        ready: !!reason,
        reason,
        bodyChildCount,
    ...
    emitScopedExtractionAudit('readiness_wait_start', {
      finalUrl,
      maxWaitMs: 3000
    });
    let readiness = await readScopedReadiness();
    if (!readiness.ready) {
      try {
        await page.waitForFunction(() => {
          const clean = value => String(value || '').replace(/\s+/g, ' ').trim();
          ...
          if (document.querySelector('h1,h2')) return true;
          if (document.querySelector('main,article,[role="main"]')) return true;
          try {
            return Array.from(document.querySelectorAll('a[href]')).slice(0, 200).some((a) => {
              const link = new URL(a.getAttribute('href') || '', location.href);
        ...
        }, null, { timeout: 3000, polling: 250 });
      } catch (_) {}
      readiness = await readScopedReadiness();
    }
    emitScopedExtractionAudit('readiness_wait_end', Object.assign({
      finalUrl,
